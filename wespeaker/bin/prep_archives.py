            ...
            if len(key_all) >= ark_size:
                ...
                with tarfile.open(tar_file, "w") as tar:
                    for n in range(ark_size):
                        ...
                        if n == 3:
                            #os.mkdir( os.path.dirname(sanity_dir + "/" + lab_file) )
                            #Path( os.path.dirname(sanity_dir + "/" + lab_file) ).mkdir(parents=True, exist_ok=True)
                            Path( sanity_dir + "/" + lab_file ).parent.mkdir(parents=True, exist_ok=True)
                            #with open(sanity_dir + "/" + spk_file, "w") as f:
                            #    f.write( spk.decode("utf8") )
                            np.save(sanity_dir + "/" + feat_file, feat)
                            np.save(sanity_dir + "/" + lab_file, lab)

                #spk_all  = spk_all[ ark_size: ] 
