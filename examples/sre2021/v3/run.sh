...
gpus="[]"
num_avg=10
checkpoint="exp/ResNet34-TSTP-emb256-fbank80-num_frms200-aug0.6-spTrue-saFalse-ArcMargin-SGD-epoch150/models/model_2.pt"
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    # Combine CTS and VoxCeleb to the final training set
...
gpus="[]"  # If we set it to an empty list, each proc will claim it gpu with safegpu
num_gpus=4
#gpus="[0,1,2,3]"
...
#gpus="[1,2,3]"  # If we set it to a nonempty list, these gpus will be used 
#num_gpus=$(echo $gpus | awk -F ',' '{print NF}')

#ls /mnt/scratch/tmp/rohdin/wespeaker/ark/* > data/train/shard.list
