...
# Note: The rate option is probably not needed. Hence currently not used below.
...
if [ "A" == "A" ];then
    ...
    for ((i=0; i<nj; i++));do
	{
	    ...
	    awk -v s=$remove_prefix_wav -v w=$wav_dir '{o=$2; sub(s, w, $2); print "sox " o  " -t gsm - | sox -t gsm - -t wav -c 1 -e signed-integer " $2    }' $slice > $dest_dir/sox_cmd_$n
...
# Some sanity checks. This may take quite some time.
